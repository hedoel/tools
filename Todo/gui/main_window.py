...
class ScraperThread(QThread):
    ...
    finished_signal = pyqtSignal(bool, str, str, list)  # 成功标志, 员工姓名, 错误信息, 记录列表

    def __init__(self, emp_id: str, password: str, year: int, month: int, rules: dict, headless: bool = True):
        super().__init__()
        self.emp_id = emp_id
    ...
    def run(self):
        try:
            ...
            self.cdp_mgr = EdgeCDPManager(port=9222, headless=True)
            ...
            emp_name, records = scraper.scrape_month_records(
                year=self.year,
                month=self.month,
                calc_rules=self.rules
            )
            
            self.finished_signal.emit(True, emp_name, "", records)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.rule_tip = QLabel("核算规则：周内18:00后30分钟起算(0.5h颗粒度)，最晚计至次日02:00 | 周末以审批加班单有效时长计算 | 法定节假日(节)不计入加班")
        ...
        self.btn_toggle_pwd.setFixedWidth(56)
        ...
        self.btn_mock.setFixedHeight(40)
        ...
        action_layout.addLayout(prog_box, 1)
        ...
        self.card_holiday = self.create_kpi_card("法定节假日(不计)", "0.0", "小时", "#64748B", "#F8FAFC", "#E2E8F0")
        self.card_allowance = self.create_kpi_card("加班津贴", "0", "元", "#7C3AED", "#FAF5FF", "#E9D5FF", sub_tag="等级 1")
        ...
        col_widths = [50, 100, 65, 125, 125, 130, 200, 100, 270]
    ...
    def init_tray(self):
        ...
        tray_menu = QMenu()
        act_show = QAction("显示主界面 (Show)", self)
        act_show.triggered.connect(self.show_and_activate)

        # 屏幕常亮防休眠快捷子菜单
        menu_awake = QMenu("☕ 屏幕常亮防休眠 (Awake)", self)
        self.act_awake_indefinite = QAction("持续常亮 (Indefinite)", self, checkable=True)
        self.act_awake_1h = QAction("保持 1 小时 (1 Hour)", self, checkable=True)
        self.act_awake_2h = QAction("保持 2 小时 (2 Hours)", self, checkable=True)
        self.act_awake_4h = QAction("保持 4 小时 (4 Hours)", self, checkable=True)
        self.act_awake_8h = QAction("保持 8 小时 (8 Hours)", self, checkable=True)
        self.act_awake_off = QAction("关闭防息屏 (Disabled)", self, checkable=True)
        ...
        act_settings = QAction("⚙️ 首选项设置 (Settings)", self)
        act_settings.triggered.connect(self.open_settings_dialog)
        act_quit = QAction("退出程序 (Exit)", self)
    ...
    def update_kpi_cards_theme(self, is_dark: bool):
        ...
        for card, text_c, bg_c, border_c in card_configs:
            ...
            """)
            card.lbl_val.setStyleSheet(f"color: {text_c};")
            card.lbl_title.setStyleSheet(f"font-size: 12px; color: {'#CBD5E1' if is_dark else '#475569'}; font-weight: bold;")
            card.lbl_unit.setStyleSheet(f"font-size: 12px; color: {'#94A3B8' if is_dark else '#64748B'}; margin-top: 6px;")
            if hasattr(card, 'lbl_sub'):
                card.lbl_sub.setStyleSheet(f"font-size: 10px; color: {text_c}; font-weight: bold; background-color: rgba(0,0,0,{'0.3' if is_dark else '0.05'}); padding: 1px 5px; border-radius: 3px;")
    ...
    def create_kpi_card(self, title: str, val: str, unit: str, text_color: str, bg_color: str, border_color: str, sub_tag: str = "") -> QFrame:
        frame = QFrame()
        ...
        lbl_title.setStyleSheet("font-size: 12px; color: #475569; font-weight: bold;")
        top_box.addWidget(lbl_title)
        top_box.addStretch()
        
        lbl_sub = QLabel(sub_tag)
        lbl_sub.setStyleSheet(f"font-size: 10px; color: {text_color}; font-weight: bold; background-color: rgba(0,0,0,0.05); padding: 1px 5px; border-radius: 3px;")
        if not sub_tag:
        ...
        val_box = QHBoxLayout()
        lbl_val = QLabel(val)
        lbl_val.setFont(QFont("JetBrains Mono", 18, QFont.Bold))
        lbl_val.setStyleSheet(f"color: {text_color};")
        
        lbl_unit = QLabel(unit)
        lbl_unit.setStyleSheet("font-size: 12px; color: #64748B; margin-top: 6px;")
    ...
    def save_current_config(self):
        self.config["emp_id"] = self.input_emp_id.text().strip()
        self.config["remember_emp_id"] = self.chk_remember_id.isChecked()
        self.config["remember_password"] = self.chk_remember_pwd.isChecked()
        # 密码经 Windows DPAPI 加密后落盘，config.json 中不再出现明文
        set_saved_password(
            self.config,
            self.input_password.text() if self.chk_remember_pwd.isChecked() else ""
        )
        save_config(self.config)
    ...
    def start_scraping(self):
        ...
        # 始终保持后台静默无头模式
        self.worker_thread = ScraperThread(emp_id, password, year, month, rules, headless=True)
    ...
    def on_worker_progress(self, msg: str, pct: int):
        if 0 < pct < 100:
            self.lbl_status.setText(f"Loading... [ {pct}% ]")
            self.lbl_status.setStyleSheet("font-weight: bold; color: #2563EB;")
        else:
    ...
    def on_password_changed(self):
        if not self.chk_remember_pwd.isChecked():
            set_saved_password(self.config, "")

    def on_emp_id_changed(self):
        if not self.chk_remember_id.isChecked():
            self.config["emp_id"] = ""

    def on_worker_finished(self, success: bool, emp_name: str, error_msg: str, records: list):
    ...
    def update_table_and_kpis(self, records: List[Dict[str, Any]]):
        ...
        self.card_allowance.lbl_sub.setVisible(True)
    ...
    def export_excel(self):
        ...
        try:
            date_range_str = f"{year}年{month:02d}月01日 至 {year}年{month:02d}月"
