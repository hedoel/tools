...
def calculate_daily_overtime(
    ...
    has_slot1_punch: Optional[bool] = None
) -> Dict[str, Any]:
    """
    计算单日加班时长
    ...
    全新三级核算规则:
    1. 法定节假日判定 (日历带“节”角标):
       - 有效加班时长不计入该月常规加班 (计 0.0 小时)
    2. 无“节”标识，看【时段 1】(常规排班出勤):
       - 若时段 1 有考勤打卡 (包括周一至周五正常工作日，以及周六日调休上班，如 5.9、9.20):
         白天标准工时 08:30-17:30 属于正常出勤 (不计加班)；
         仅按 18:00 之后的延时打卡核算加班 (18:00 前下班计 0.0h)。
    3. 无“节”标识且【时段 1】无考勤打卡 (公休/非排班出勤):
       - 看是否有审批的【有效加班时长】:
         有审批单且时长 > 0 则按审批单有效时长计入周末加班；
         无有效加班单或时长为 0 则计 0.0 小时。
    """
    ...
    # 推导时段 1 是否有考勤打卡
    if has_slot1_punch is None:
        if is_public_holiday_shift:
            has_slot1_punch = False
        elif is_weekend and (ot_valid_hours is not None and ot_valid_hours > 0):
            # 周末且有审批加班单时，打卡属于加班单出勤，非正常排班班次
            has_slot1_punch = False
        else:
            has_slot1_punch = bool(check_in_dt or check_out_dt)
    ...
    # 规则 2: 无“节”标识，且【时段 1】有打卡（排班正常出勤，含平时与调休上班）
    if has_slot1_punch:
        if not check_out_dt:
            result["notes"] = "下班未打卡或无出勤"
            result["detail_type"] = "正常工时"
            return result
        ...
        prefix = "调休上班，" if is_weekend else ""
    ...
    # 规则 3: 无“节”标识且【时段 1】无打卡（公休/休息日）
    # 先看是否有审批的【有效加班时长】
    if ot_valid_hours is not None and ot_valid_hours > 0:
    ...
    if ot_valid_hours is not None and ot_valid_hours == 0:
        ...
        result["notes"] = "周末加班审批单有效时长为 0.0小时"
        return result
    ...
    # 如果下班未打卡或没有打卡记录
    if not check_out_dt:
        result["notes"] = "无时段1考勤打卡且无有效加班审批单"
        result["detail_type"] = "正常工时"
        return result

    # 兜底：既无时段1打卡也无审批单，但打卡了且非公休且开启了兜底配置
    if is_weekend and include_weekend_base:
        ot_start_dt, ot_cutoff_dt = resolve_ot_window(record_date, weekday_ot_start, ot_latest_end)
        ot_hours, ot_minutes, dropped_minutes = _calc_ot_hours(
            ot_start_dt, check_out_dt, ot_cutoff_dt, min_ot_minutes, granularity_minutes
        )
