...
def format_shift_name(shift: Optional[str]) -> str:
    """班次名称精简：支持全角/半角括号及冗余前缀剔除，保留核心班次名称（如：白班、公休白班、公休等）"""
    if not shift:
        return ""
    s = str(shift)
    for sep in ["(", "（", "[", "【"]:
        if sep in s:
            s = s.split(sep)[0]
    for prefix in ["SD1-", "SD2-", "SD3-", "SD4-", "SD5-", "职员"]:
        s = s.replace(prefix, "")
    s = s.strip()
    if len(s) > 4:
        s = s[:4]
...
class DayCellWidget(QFrame):
    ...
    def init_ui(self):
        ...
        # 班次名称精简：支持全角/半角括号及冗余前缀剔除，保留核心班次名称（如：白班、公休白班、公休等）
        short_shift = format_shift_name(shift)
